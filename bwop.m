...
for i=dt/2:dt:20,
    ...
    if doanimation, %animation code
        ...
        axis equal;axis([max(4,p(1,1))+[-6 3] -1 6]); drawnow;
    end;
    if any(contact([1 6])), break; end %hip or head hit the ground, so stop
end
rating=-p(1,1); %return (negative) x coordinate of hip
%v is the velocity of each part of Tobias; the first entry is the hip, 6th
...
if ~doanimation %draw time-lapse version otherwise
    if (now-lastCalled)*60*60*24>0.25, %have we been called in the last quarter second?
        ...
        axis equal;axis([-2 10 -1 6]); drawnow;
    end
end    
